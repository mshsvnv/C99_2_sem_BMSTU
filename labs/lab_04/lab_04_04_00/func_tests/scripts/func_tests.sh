...
pos_tests() {
	...
	pos_args=""
	if [[ -f ../data/pos_"$i"_args.txt ]]; then
		pos_args=$(ls ../data/pos_"$i"_args.txt)
	...
	if [[ "$exit_code" -eq 0 ]]; then 
		...
		if [[ "$exit_code" -eq 1 ]]; then
			...
			if [[ "$exit_code" -eq 10 ]]; then
				echo -e "POS TEST pos_${i}_in.txt:$green PASSED""$end"
				echo -e "POS TEST pos_${i}_in.txt:$yel MEMMORY ERROR$end"
				pos_pass=$((pos_pass+1))
			fi
			...
			if [[ "$exit_code" -eq 30 ]]; then
				echo -e "POS TEST pos_${i}_in.txt:$green PASSED""$end"
				echo -e "POS TEST pos_${i}_in.txt:$green MEMMORY OK""$end"
...
neg_tests() {
	...
	neg_args=""
	if [[ -f ../data/neg_"$i"_args.txt ]]; then
		neg_args=$(ls ../data/neg_"$i"_args.txt)
		./neg_case.sh "$neg_in" "$neg_out" "$neg_args"
	else
		./neg_case.sh "$neg_in" "$neg_out" 
	fi
	...
	if [[ "$exit_code" -eq 0 ]]; then 
		...
		if [[ "$exit_code" -eq 1 ]]; then
			...
			if [[ "$exit_code" -eq 10 ]]; then
				echo -e "NEG TEST neg_${i}_in.txt:$green PASSED""$end"
				echo -e "NEG TEST neg_${i}_in.txt:$yel MEMMORY ERROR$end"
				neg_pass=$((neg_pass+1))
			fi
			if [[ "$exit_code" -eq 20 ]]; then
				echo -e "NEG TEST neg_${i}_in.txt:$red FAILED$end"
			...
			if [[ "$exit_code" -eq 30 ]]; then
				echo -e "NEG TEST neg_${i}_in.txt:$green PASSED""$end"
				echo -e "NEG TEST neg_${i}_in.txt:$green MEMMORY OK""$end"
				neg_pass=$((neg_pass+1))
...
pos_pass_mem=0
while [ -f ../data/pos_"$i"_in.txt ] && [ -f ../data/pos_"$i"_out.txt ]; do

	pos_in=$(ls ../data/pos_"$i"_in.txt)
	pos_out=$(ls ../data/pos_"$i"_out.txt)

	pos_tests "$pos_in", "$pos_out", "$i"
...
if [[ "$USE_VALGRIND" != "" ]]; then
	echo -e "${bold_white}Memmory:$end correct $pos_pass_mem of $((j-1))\n"
fi
...
neg_pass_mem=0
while [ -f ../data/neg_"$i"_in.txt ] && [ -f ../data/neg_"$i"_out.txt ]; do

	neg_in=$(ls ../data/neg_"$i"_in.txt)
	neg_out=$(ls ../data/neg_"$i"_out.txt)

	neg_tests "$neg_in", "$neg_out", "$i"
...
if [[ "$USE_VALGRIND" != "" ]]; then
	echo -e "${bold_white}Memmory:$end correct $neg_pass_mem of $((j-1))\n"
fi
