...
pos_tests() {
	...
	./pos_case.sh "$pos_args" "$pos_in" "$pos_out" 
...
pos_pass_mem=0
while [[ -f ../data/pos_"$i"_args.txt ]]; do

	pos_args=$(ls ../data/pos_"$i"_args.txt)

	pos_in=""
	if [[ -f ../data/pos_"$i"_in.txt ]]; then
		pos_in=$(ls ../data/pos_"$i"_in.txt)
	fi

	pos_out=""
	if [[ -f ../data/pos_"$i"_out.txt ]]; then
		pos_out=$(ls ../data/pos_"$i"_out.txt)
	fi

	pos_tests "$pos_args", "$i", "$pos_in", "$pos_out"
...
neg_pass_mem=0
while [[ -f ../data/neg_"$i"_args.txt ]]; do

	neg_args=$(ls ../data/neg_"$i"_args.txt)

	neg_in=""
	if [[ -f ../data/neg_"$i"_in.txt ]]; then
		neg_in=$(ls ../data/neg_"$i"_in.txt)
	fi

	neg_tests "$neg_args", "$i", "$neg_in"
